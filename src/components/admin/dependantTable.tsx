...
import { DependantColumns } from "./columns";
import { api } from "~/trpc/react";

const DependantTable: FunctionComponent = () => {
  const { data, fetchNextPage, isFetchingPreviousPage, isFetchingNextPage } =
    api.admin.getAllDependants.useInfiniteQuery(
      { take: 50 },
  ...
  const currPage = data?.pages[pageNo]?.passes;
